...
<?php
	...
	if( !empty( $share_items ) ) {
		...
		$my_query = new Network_Query( 'posts_per_page='.$sc_options['number_post'].'' );

	elseif ( $sc_options['posts_slider']=='2' ) :

		$my_query = new Network_Query( 'orderby=comment_count&posts_per_page='.$sc_options['number_post'].'' );

	elseif ( $sc_options['posts_slider']=='3' ) :

		$my_query = new Network_Query( 'category_name='.$sc_options['category_slider'].'&posts_per_page='.$sc_options['number_post'].'' );
	...
	elseif ( $sc_options['posts_slider']=='4' ) :
		...
		$args = $str;
		$my_query = new Network_Query( array( 'post_type' => 'post', 'post__in' => $str ) );

	elseif ( $sc_options['posts_slider']=='5' ) :

		$my_query = new Network_Query( 'post_type=page&posts_per_page='.$sc_options['number_post'].'' );
	...
	elseif ( $sc_options['posts_slider']=='6' ) :
		...
		$str = explode(',', $sc_options['posts_id']);
		$args = $str;
		$my_query = new Network_Query( array( 'post_type' => 'page', 'post__in' => $str ) );
	...
	elseif ( $sc_options['posts_slider']=='7' ) :
		...
		$str = explode(',', $sc_options['posts_id']);
		$args = $str;
		$my_query = new Network_Query( array( 'post_type' => $str, 'posts_per_page' => $sc_options['number_post'] ) );
